...
def test_search_data_no_results(setup_directories, capsys):
    """Teste la méthode search_data lorsqu'aucun résultat n'est trouvé."""
    input_dir, _ = setup_directories
    file_path = input_dir / "empty_search.csv"

    # Créer un fichier CSV valide avec des données non correspondantes
    with open(file_path, "w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["name", "category", "price", "quantity"])
        writer.writeheader()
        writer.writerows([
    ...
    # Rechercher une requête qui ne correspond à aucun résultat
    commerce.search_data(file_path=str(file_path), query="Nonexistent", category="Category 3", price_range="50,100")
